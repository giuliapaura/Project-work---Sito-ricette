...
// echo $jwt;
try {
    $sql = "UPDATE users SET token=? WHERE username=? AND password=?";
...
try {
    $sql = 'SELECT token, scadenza, username, idUser FROM users WHERE username = :username AND password = :password';
    $stmt = $dbh->prepare($sql);
